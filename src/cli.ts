...
program
  ...
  .action(async (options) => {
    ...
    // Generate embedded assets file before building
    const generateEmbeddedAssets = (assetsDir: string): Record<string, string> => {
      ...
      walk(assetsDir, assetsDir);
      return assets;
    };

    const embeddedAssets = generateEmbeddedAssets(keystoneDir);
    const generatedFilePath = resolve(import.meta.dir, 'generated/embedded-assets.ts');
    const generatedContent = `// Auto-generated by 'keystone compile' - DO NOT EDIT
export const EMBEDDED_ASSETS: Record<string, string> = ${JSON.stringify(embeddedAssets, null, 2)};
`;
    const { writeFileSync } = await import('node:fs');
    writeFileSync(generatedFilePath, generatedContent);
    console.log(`📝 Generated embedded assets (${Object.keys(embeddedAssets).length} files)`);
